...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $user = User::with(['studentProfile', 'academicSubjects', 'careerMatches.career'])
                   ->first();
        ...
        $totalQuestions = 40;
        $answeredQuestions = InterestAssessment::where('user_id', $user->id)
                                             ->whereNotNull('answer')
                                             ->count();
        $assessmentProgress = $totalQuestions > 0 ? round(($answeredQuestions / $totalQuestions) * 100) : 0;

        // Get primary career goal
        $primaryCareer = CareerMatch::with('career')
                                  ->where('user_id', $user->id)
                                  ->where('is_primary_goal', true)
                                  ->first();

        // Get upcoming deadlines
        $upcomingDeadlines = Deadline::with('application')
                                   ...
                                   ->map(function ($deadline) {
                                       return [
                                           ...
                                           'daysLeft' => (int) $deadline->days_remaining,
                                           'status' => $deadline->urgency_level === 'Critical' ? 'urgent' : 'normal',
                                           'dueDate' => $deadline->due_date ? $deadline->due_date->format('M d, Y') : 'TBD'
                                       ];
                                   });

        // Get interest fit data (mock for now - will be calculated from assessment)
        $interestFit = [
            ['category' => 'Investigative', 'description' => 'Research & Analysis', 'percentage' => 92],
            ['category' => 'Conventional', 'description' => 'Organized & Detail-oriented', 'percentage' => 78],
            ['category' => 'Realistic', 'description' => 'Practical & Hands-on', 'percentage' => 65]
        ];
        ...
        $dashboardData = [
            'user' => [
                ...
                'studentType' => $user->current_education_level ?? 'SPM Student',
                'academicSubjects' => $user->academicSubjects->map(function ($subject) {
                    return [
                        'name' => $subject->subject_name,
                        'grade' => $subject->grade,
                        'color' => $subject->grade_color
                    ];
                })
            ],
            'assessment' => [
            ...
            'careerGoal' => $primaryCareer ? [
                'title' => $primaryCareer->career->title,
                'matchPercentage' => $primaryCareer->match_percentage
            ] : null,
            'pathwayPlan' => [
                'route' => 'Main Route',
                'path' => 'STPM → Computer Science'
            ],
            'simulations' => [
                'count' => 3,
                'description' => 'Scenarios Created'
            ],
        ...
        $dashboardData['navigationItems'] = [
            ['id' => 1, 'name' => 'Dashboard', 'href' => '/dashboard', 'icon' => 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2 2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6', 'active' => true],
